export default {
    ...
    mutations: {
        ...
        onChange(state, par) {
            var _curd = state.curd;
            this.dispatch(`vuex${_curd.controllerName}/save`, {
                RoleId: _curd.tree.roleid,
                MenuId: par.id,
                FunctionIds: par.values
            });
        },
        // expand(state, par) {
        //     var _curd = state.curd;
        //     console.log('欧耶:', par);
        //     // _curd.tree.vm.expandedRowKeys = par;
        // }
    },
    actions: {
        //获取数据表格
        ...
        save(context, par) {
            ...
            //发送请求给接口
            global
                .post(`Admin/${_curd.controllerName}/Save`, par, true)
                .then(res => {
                    var data = res.data.data;
                    //刷新列表
                    context.dispatch("roleMenuFunctionTree", _curd.tree.roleid);
                    global.tools.msg('操作成功!', '成功');
                });
        },
        //导出Excel
        ...
        roleMenuFunctionTree(context, par) {
            var _curd = context.state.curd;
            _curd.tree.roleid = par;
            _curd.table.selectedRowKeys = [par];
            //发送请求给接口
            global
                .post(`Admin/${_curd.controllerName}/RoleMenuFunctionTree/${_curd.tree.roleid}`, null, false)
                .then(res => {
                    var data = res.data.data;
                    _curd.tree.vm = data;
                });
