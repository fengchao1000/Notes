export default {
    ...
    actions: {
        ...
        findList(context, par) {
            ...
            global
                ...
                .then(res => {
                    ...
                    _curd.table.columns =[
                        {DataIndex:'TableID',Title:'编号',Show:true},
                        {DataIndex:'TableName',Title:'表格名称',Show:true},
        ...
        save(context, par) {
            ...
            var _vm = _form.vm;
            //验证数据
            if (!_vm.Model.TableName) return global.tools.msg('用户名不能为空!', '错误');
            if (_curd.columnLst.columns.length<1) {
                return global.tools.msg('请至少添加一列!', '错误');
            }
            _vm.Model.Columns=JSON.stringify(_curd.columnLst.columns);
            //发送请求给接口
            global
