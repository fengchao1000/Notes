      ...
      <template slot="tools">
        ...
        <a-row :gutter="20">
          <a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="pb-20">
            ...
            <a-popconfirm title="删除时将同时删除数据表，继续?" @confirm="remove()" okText="确定" cancelText="取消">
      ...
      <template slot="form">
        ...
        >
          <a-form :form="curd.form.vm" layout="horizontal" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }" hideRequiredMark :selfUpdate="true">
            <a-row :gutter="20">
              <a-col >
                <a-form-item label="彩票名称">
                  <a-input v-model="curd.form.vm.Model.LotName" placeholder="彩票名称" />
                </a-form-item>
              </a-col>
            </a-row>
             <a-row :gutter="20">
              <a-col >
                <a-form-item label="数据表名称">
                  <a-input v-model="curd.form.vm.Model.DataTableName" placeholder="数据表名称" />
                </a-form-item>
              </a-col>
            </a-row>
             ...
             <a-row :gutter="20">
              <a-col >
                <a-form-item label="选择分类">
                  <a-select   v-model="curd.form.vm.Model.CateID" >
                       <a-select-option v-for="(item) in curd.form.vm.cate" :key="item.ID" :value="item.ID">{{item.CateName}}</a-select-option>
                   </a-select>
                </a-form-item>
              </a-col>
            </a-row>
            ...
            <a-row :gutter="20">
              <a-col >
                <a-form-item label="选择类型">
                  <a-select   v-model="curd.form.vm.Model.LTypeID" >
                       <a-select-option v-for="(item) in curd.form.vm.types" :key="item.ID" :value="item.ID">{{item.TypeName}}</a-select-option>
                   </a-select>
                </a-form-item>
              </a-col>
            </a-row>
            <a-row :gutter="20">
              <a-col >
                <a-form-item label="球号类型">
                  <a-select  v-model="curd.form.vm.Model.BallDisplayType" >
                       <a-select-option value="赛车">赛车</a-select-option>
                       <a-select-option value="福彩">福彩</a-select-option>
                       <a-select-option value="大乐透">大乐透</a-select-option>
                   </a-select>
                </a-form-item>
              </a-col>
            </a-row>
